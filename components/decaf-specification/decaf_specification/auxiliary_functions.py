...
def convert_bandwidth(data, reverse=False):
    ...
    :param data: dictionary bottle.FormsDict variable to be checked. None or empty is consideted valid.
    :param reverse: by default convert form str to int (Mbps), if True it convert from number to units.
    :return: None
    """
                ...
                if not reverse:
                    ...
                    if pos > 0:
                        if value[pos - 1] == "G":
                            ...
                            data["bandwidth"] = int(data["bandwidth"][:pos - 1])
                else:
...
def convert_str2boolean(data, items):
    ...
    :param data': Dictionary variable to be checked. None or empty is considered valid.
    :param items: Tuple of keys to convert.
    :return: None
    """
    if type(data) is dict:
        for k in data.keys():
            ...
            if k in items:
                if type(data[k]) is str:
                    if data[k] == "false" or data[k] == "False":
                        data[k] = False
                    elif data[k] == "true" or data[k] == "True":
                        data[k] = True
    if type(data) is tuple or type(data) is list:
        for k in data:
            if type(k) is dict or type(k) is tuple or type(k) is list:
...
def generate_queue_name(pluginname):
    """
    This function generate the queue name of a plugin.
    The queue name is composed of pluginname + uuid (This uuid rely on the timestamp.
    and therefore can not appear twice).

    :param pluginname: The name of the plugin, which requires a queue name.
    :return: pluginname + generateduuid.
    """
    generateduuid = str(myUuid.uuid1())
    generateduuid = generateduuid.split("-", 4)[0]
    return pluginname + generateduuid
